...
type GoalsPerDay = Record<string, Array<{
  id: string
  title: string
  completedAt: Date
}>>
...
export async function getWeekSummary() {
  ...
  const goalsCreatedUpToWeek = db.$with('golas_created_up_to_week').as(
    db
      .select({
        id: goals.id,
        title: goals.title,
  ...
  const goalsCompletedByWeekDay = db.$with('goals_completed_by_week_day').as(
    db
      .select({
        ...
        `.as('completions'),
        completionCount: count(goalsCompletedInWeek.id)
          .as('completionCount'),
      })
  ...
  // .from(goalsCompletedByWeekDay).toSQL()
